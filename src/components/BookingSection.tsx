...
import { SERVICES, isDayAllowed, WHATSAPP_NUMBER, generateWhatsAppUrl, formatPhone, getBookingDuration, ScheduleBlock, generateUUID } from '@/lib/types';
import { addBooking, getBookings, getBlocks, getScheduleSettings } from '@/lib/bookingStore';
import { motion, AnimatePresence } from 'framer-motion';
import { Calendar } from '@/components/ui/calendar';
import { cn } from '@/lib/utils';
import { format } from 'date-fns';
import { ptBR } from 'date-fns/locale';
import { CheckCircle, X, Plus, Copy, Check, Upload, FileImage } from 'lucide-react';
...
const BookingSection = () => {
  ...
  useEffect(() => {
    ...
    // Ler as configurações dinâmicas
    const scheduleSettings = getScheduleSettings();
    const dayOfWeek = selectedDate.getDay();
    // Se existir uma configuração para a data específica, usa ela. Senão, usa a configuração da semana.
    const baseTimes = scheduleSettings.specificDates[dateStr] || scheduleSettings.weekly[dayOfWeek] || [];
  ...
  const finishBookingFlow = (booking: any) => {
    ...
    fetch('/api/calendar', {
      ...
      body: JSON.stringify({ type: 'booking', booking: booking, duration: totalDuration }),
    })
      .then(async (res) => {
